...
import { deleteSnorkelingPackage } from "@/app/actions/snorkeling";
...
export default function SnorkelingTable({ packages }: Props) {
  ...
  function handleDelete(id: string, name: string) {
    ...
    startTransition(async () => {
      try {
        await deleteSnorkelingPackage(id);
      } catch (err: any) {
          ...
          {packages.length === 0 ? (
            <tr>
              <td colSpan={7} className="px-6 py-10 text-center text-gray-400">
                Belum ada paket snorkeling. Klik "+ Tambah Paket" untuk mulai.
              </td>
            </tr>
          ) : (
            packages.map((p) => (
              <tr key={p.id} className="hover:bg-gray-50/30 transition-colors">
                ...
                <td className="px-6 py-4">
                  <div className="flex items-center gap-3">
                    ...
                    <Link
                      href={`/admin/snorkeling/edit/${p.id}`}
