  ...
  return (
    <div className="pt-16 lg:pt-20 pb-20">
      <section className="relative h-64 flex items-center justify-center overflow-hidden mb-12">
        <Image src="https://images.unsplash.com/photo-1549488344-cbb6c34cf08b?q=80&w=1920&auto=format&fit=crop" alt="Speedboat" fill sizes="100vw" className="object-cover" />
        <div className="absolute inset-0 bg-linear-to-r from-gili-900/80 to-gili-700/60" />
        <div className="relative z-10 text-center text-white">
          <h1 className="text-4xl lg:text-5xl font-bold mb-3">{t.nav.speedboat}</h1>
          <p className="text-gili-200 text-lg">{locale === "en" ? "Private speedboat for an exclusive adventure" : "Private speedboat untuk petualangan eksklusif"}</p>
        </div>
      </section>
          ...
          {allSpeedboats.map((sb) => (
            <div key={sb.id} className="bg-white rounded-2xl shadow-lg overflow-hidden hover:shadow-xl transition-shadow border border-gray-100">
              <div className="relative h-56 overflow-hidden">
                <Image src={sb.images[0]} alt={sb.name} fill sizes="(max-width: 768px) 100vw, 50vw" className="object-cover" />
              ...
              <div className="p-6 space-y-4">
                ...
                <div>
                  <p className="text-xs text-gray-400 font-semibold mb-2">{locale === "en" ? "Routes:" : "Rute:"}</p>
                ...
                <div>
                  <p className="text-xs text-gray-400 font-semibold mb-2">{locale === "en" ? "Facilities:" : "Fasilitas:"}</p>
                  <div className="flex flex-wrap gap-2">
                    {sb.facilities.map((f) => (
                ...
                <div className="flex items-end justify-between pt-4 border-t border-gray-100">
                  <div>
                    <p className="text-2xl font-bold text-gili-600">{formatCurrency(sb.price, locale)}</p>
