...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const pkg = await getActivityBySlug(slug, "private-speed-boat-and-car");
  if (!pkg) return {};
  return {
    title: `${pkg.name} | Transportasi Privat`,
    description: pkg.description,
  };
}

export default async function SpeedboatCarDetailPage({ params }: Props) {
  const { slug } = await params;
  const pkg = await getActivityBySlug(slug, "private-speed-boat-and-car");
