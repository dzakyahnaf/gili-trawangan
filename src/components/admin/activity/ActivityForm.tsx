...
export default function ActivityForm({ initialData, category, categoryLabel }: Props) {
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    startTransition(async () => {
      try {
        if (initialData?.id) {
          await updateActivity(initialData.id, category, fd);
        } else {
          await createActivity(category, fd);
        }
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6">
      <div className="flex items-center gap-3">
        <Link href={`/admin/${category}`} className="p-2 rounded-xl hover:bg-gray-100 transition-colors">
          <ArrowLeft className="w-5 h-5 text-gray-600" />
        </Link>
        <div>
          <h1 className="text-2xl font-bold text-gray-900">
            {initialData ? `Edit Paket ${categoryLabel}` : `Tambah Paket ${categoryLabel} Baru`}
          </h1>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="grid md:grid-cols-2 gap-4">
            <div>
              ...
              <input type="text" value={name} onChange={(e) => setName(e.target.value)}
                placeholder="Contoh: Paket Wisata Spesial" className={inputCls} required />
            </div>
            <div>
              <label className={labelCls}>Package Name 🇬🇧</label>
              <input type="text" value={nameEn} onChange={(e) => setNameEn(e.target.value)}
                placeholder="Example: Special Tour Package" className={inputCls} required />
            </div>
            <div>
          ...
          <div className="grid md:grid-cols-2 gap-4">
            <div>
              ...
              <textarea value={highlights} onChange={(e) => setHighlights(e.target.value)}
                rows={4} placeholder={"Highlight 1\nHighlight 2"}
              ...
              <div key={idx} className="p-4 bg-gray-50 rounded-xl border border-gray-100 space-y-3">
                ...
                <div className="grid md:grid-cols-2 gap-3">
                  ...
                  <div className="space-y-2">
                    ...
                    <input type="text" value={item.time || ""} placeholder="Waktu/Hari (cth: 09:00 atau Hari 1)"
                  ...
                  <div className="space-y-2">
                    ...
                    <input type="text" value={itineraryEn[idx]?.time || ""} placeholder="Time/Day (e.g. 09:00 or Day 1)"
          ...
          <div className="grid sm:grid-cols-2 md:grid-cols-3 gap-4">
            ...
            <div>
              ...
              <input type="text" value={duration} onChange={(e) => setDuration(e.target.value)}
                placeholder="cth: 4 Jam / 2 Hari 1 Malam" className={inputCls} required />
            </div>
            <div>
              <label className={labelCls}>Jadwal (Satu per baris)</label>
              <textarea value={schedule} onChange={(e) => setSchedule(e.target.value)}
                rows={2} placeholder={"Setiap Hari\n09:00"} className={textareaCls} />
            </div>
            <div>
              <label className={labelCls}>Price Unit / Info Tambahan (di tampilkan di harga)</label>
              <input type="text" value={meetingPoint} onChange={(e) => setMeetingPoint(e.target.value)}
                placeholder="cth: per way / /pax / per trip" className={inputCls} />
            </div>
            <div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 space-y-4">
          ...
          <div>
            <label className={labelCls}>Atau gunakan path lokal (cth: /images/rinjani-tracking1.jpg)</label>
            <input type="text" name="localImagePath" placeholder="/images/rinjani-tracking1.jpg"
